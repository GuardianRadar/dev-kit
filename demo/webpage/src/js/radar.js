...
function drawBaseMap(ctx) {
    ...
    for (let i = 90 % majorAngleAmount; i < 180; i += majorAngleAmount) {
        ...
        ctx.lineTo(width / 2 + height * Math.cos(radians), height - height * Math.sin(radians));
        ctx.stroke();
        ctx.closePath();
    }
    ...
    ctx.lineWidth = 1;
    ctx.strokeStyle = "#555";
    for (let i = minorMetersAmount; i < mapProperties.maxRange; i += minorMetersAmount) {
        ctx.beginPath();
    ...
    for (let i = majorMetersAmount; i < mapProperties.maxRange; i += majorMetersAmount) {
        ctx.beginPath();
        ctx.arc(width / 2, height, i / m_per_px, 0, Math.PI, true);
        ctx.stroke();
        ctx.closePath();
    ...
    for (let i = 90 % minorAngleAmount; i <= 180; i += minorAngleAmount) {
        const radians = (i * Math.PI) / 180;
        ctx.fillText(`${90 - i}`, width / 2 + (height - textPadding) * Math.cos(radians), height - (height - textPadding) * Math.sin(radians));
    }
    for (let i = majorMetersAmount; i < mapProperties.maxRange; i += majorMetersAmount) {
...
function drawDetections(ctx) {
    ...
    for (const detection of radarOutputs.detections) {
        ...
        ctx.beginPath();
        ctx.arc(x, y, (radarProperties.rangeResolution / 2) * mp.px_per_m, 0, 2 * Math.PI, false);
        ctx.fill();
        ctx.closePath();
    }
    let removeAmount = 0;
...
function drawClusters(ctx) {
    ...
    for (const cluster of radarOutputs.clusters) {
        ...
        ctx.beginPath();
        ctx.moveTo(mp.width / 2 + center.x * mp.px_per_m, mp.height - center.y * mp.px_per_m);
        ctx.lineTo(mp.width / 2 + (center.x + velocity.x) * mp.px_per_m, mp.height - (center.y + velocity.y) * mp.px_per_m);
        ctx.stroke();
        ctx.closePath();
        ctx.font = "10px Arial";
        ctx.fillStyle = "white";
        ...
        for (const detection of radarOutputs.detections) {
            if (cluster.detectionIds.includes(detection.id)) {
                ...
                ctx.beginPath();
                ctx.moveTo(x, y);
                ctx.lineTo(mp.width / 2 + center.x * mp.px_per_m, mp.height - center.y * mp.px_per_m);
                ctx.stroke();
                ctx.closePath();
            }
        }
...
function drawTracks(ctx) {
    const mp = mapProperties;
    const halfDiamondSize = Math.max(10, radarProperties.rangeResolution * mp.px_per_m);
    ctx.lineWidth = 2;
    ctx.font = "10px Arial";
    for (const track of radarOutputs.tracks) {
        ...
        for (let i = 0; i < 4; ++i) {
            ...
            ctx.lineTo(mp.width / 2 + x[(i + 1) % 4], mp.height - y[(i + 1) % 4]);
            ctx.stroke();
            ctx.closePath();
        }
        ctx.beginPath();
        ctx.moveTo(mp.width / 2 + track.position.x * mp.px_per_m, mp.height - track.position.y * mp.px_per_m);
        ctx.lineTo(mp.width / 2 + (track.position.x + track.velocity.x) * mp.px_per_m, mp.height - (track.position.y + track.velocity.y) * mp.px_per_m);
        ctx.stroke();
        ctx.closePath();
        ctx.fillStyle = color;
        ctx.beginPath();
        ctx.fillText(track.id, mp.width / 2 + x[1] + halfDiamondSize, mp.height - y[0] - halfDiamondSize);
        ctx.closePath();
    }
    let removeAmount = 0;
